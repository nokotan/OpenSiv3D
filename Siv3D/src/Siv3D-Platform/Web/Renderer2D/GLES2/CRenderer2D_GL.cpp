...
# include <ConstantBuffer/GLES2/GLConstantBuffer.hpp>
...
namespace s3d
{
	...
	void CRenderer2D_GL::init()
	{
		...
		// 標準 VS をロード
		{
			Array<ConstantBufferBinding> VSUniformBindings = {
				{ U"g_transform0", 0 },
				{ U"g_transform1", 4 },
				{ U"g_colorMul", 8 },
			};
			...
			m_standardVS->sprite = VertexShader(Resource(U"engine/shader/GLSL1.0/sprite.vert"), VSUniformBindings);
			if (!m_standardVS->ok())
			{
		...
		// 標準 PS をロード
		{
			Array<ConstantBufferBinding> PSUniformBindings = {
				{ U"g_colorAdd", 0 },
				{ U"g_sdfParam", 4 },
				{ U"g_internal", 8 }
			};
			...
			m_standardPS->shape			= PixelShader(Resource(U"engine/shader/GLSL1.0/shape.frag"), PSUniformBindings);
			m_standardPS->texture		= PixelShader(Resource(U"engine/shader/GLSL1.0/texture.frag"), PSUniformBindings);
			m_standardPS->square_dot	= PixelShader(Resource(U"engine/shader/GLSL1.0/square_dot.frag"), PSUniformBindings);
			m_standardPS->round_dot		= PixelShader(Resource(U"engine/shader/GLSL1.0/round_dot.frag"), PSUniformBindings);
			m_standardPS->sdf			= PixelShader(Resource(U"engine/shader/GLSL1.0/sdf.frag"), PSUniformBindings);
	...
	void CRenderer2D_GL::flush()
	{
		...
		// pShader->setVSUniforms(m_standardVS->sprite.id(), m_vsConstants2D.base()._detail()->getData());
		// pShader->setPSUniforms(m_standardPS->shape.id(), m_psConstants2D.base()._detail()->getData());
		// pShader->setPSSamplerUniforms(m_standardPS->shape.id());
		
		Size currentRenderTargetSize = pGraphics->getSceneSize();
		Mat3x2 transform = Mat3x2::Identity();
		Mat3x2 screenMat = Mat3x2::Screen(currentRenderTargetSize);
		BatchInfo batchInfo;
		Array<TextureID> textureSlot { 8, TextureID::InvalidValue() };
		VertexShader::IDType currentVertexShaderID = m_standardVS->sprite.id();
		PixelShader::IDType currentPixelShaderID = m_standardPS->shape.id();
			...
			case RendererCommand::Draw:
				{
					...
					{
						const auto& textureID = textureSlot[slot];
					...
					pGraphics->getSamplerState()->applyState();

					m_vsConstants2D._update_if_dirty();
					m_psConstants2D._update_if_dirty();

					pShader->setVSUniforms(currentVertexShaderID, m_vsConstants2D.base()._detail()->getData());
					pShader->setPSUniforms(currentPixelShaderID, m_psConstants2D.base()._detail()->getData());
					pShader->setPSSamplerUniforms(currentPixelShaderID);
				...
				case RendererCommand::SetPS:
				{
					...
					else
					{
						...
						m_pipeline.use();
						// pShader->setVSUniforms(m_standardVS->sprite.id(), m_vsConstants2D.base()._detail()->getData());
						// pShader->setPSUniforms(psID, m_psConstants2D.base()._detail()->getData());
						// pShader->setPSSamplerUniforms(psID);

						currentPixelShaderID = psID;
					}
				...
				{
					LOG_ERROR(U"RenderCommand 'SetCB' is not currently supported.");
	...
	void CRenderer2D_GL::addRectFrame(const FloatRect& rect, const float thickness, const Float4& innerColor, const Float4& outerColor)
	{
	...
	void CRenderer2D_GL::addCirclePie(const Float2& center, const float r, const float startAngle, const float angle, const Float4& innerColor, const Float4& outerColor)
	{
	...
	void CRenderer2D_GL::addCircleArc(const Float2& center, const float rInner, const float startAngle, float angle, const float thickness, const Float4& innerColor, const Float4& outerColor)
	{
	...
	{
		CTexture_GL* const pTexture = dynamic_cast<CTexture_GL* const>(Siv3DEngine::Get<ISiv3DTexture>());
		const Vec2 potScale = pTexture->getPOTScale(texture.id());
		const FloatRect scaledUV { uv.left * potScale.x, uv.top * potScale.y, uv.right * potScale.x, uv.bottom * potScale.y };

		if (const uint16 indexCount = Vertex2DBuilder::BuildTextureRegion(m_bufferCreator, rect, scaledUV, color))
		{
			if (!m_currentCustomPS)
			{
				m_commands.pushStandardPS(texture.isSDF() ? m_standardPS->sdfID : m_standardPS->textureID);
	...
	void CRenderer2D_GL::addTextureRegion(const Texture& texture, const FloatRect& rect, const FloatRect& uv, const Float4(&colors)[4])
	{
		...
		if (const uint16 indexCount = Vertex2DBuilder::BuildTextureRegion(m_bufferCreator, rect, scaledUV, colors))
		{
			if (!m_currentCustomPS)
			{
				m_commands.pushStandardPS(texture.isSDF() ? m_standardPS->sdfID : m_standardPS->textureID);
	...
	void CRenderer2D_GL::addTexturedCircle(const Texture& texture, const Circle& circle, const FloatRect& uv, const Float4& color)
	{
		...
		if (const uint16 indexCount = Vertex2DBuilder::BuildTexturedCircle(m_bufferCreator, circle, scaledUV, color, getMaxScaling()))
		{
			if (!m_currentCustomPS)
			{
				m_commands.pushStandardPS(texture.isSDF() ? m_standardPS->sdfID : m_standardPS->textureID);
	...
	void CRenderer2D_GL::addTexturedQuad(const Texture& texture, const FloatQuad& quad, const FloatRect& uv, const Float4& color)
	{
		...
		if (const uint16 indexCount = Vertex2DBuilder::BuildTexturedQuad(m_bufferCreator, quad, scaledUV, color))
		{
			if (!m_currentCustomPS)
			{
				m_commands.pushStandardPS(texture.isSDF() ? m_standardPS->sdfID : m_standardPS->textureID);
