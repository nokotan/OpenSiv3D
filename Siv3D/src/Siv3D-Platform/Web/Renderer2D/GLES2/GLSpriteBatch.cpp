...
namespace s3d
{
	...
	void GLSpriteBatch::setVertexAttrib()
	{		
	...
	BatchInfo GLSpriteBatch::updateBuffers(const size_t batchIndex)
	{
		...
		if (const uint16 vertexSize = currentBatch.vertexPos)
		{
			...
			::glBufferSubData(GL_ARRAY_BUFFER, sizeof(Vertex2D) * m_vertexBufferWritePos, sizeof(Vertex2D) * vertexSize, pSrc);
		...
		if (const uint32 indexSize = currentBatch.indexPos)
		{
			...
			::glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, sizeof(IndexType) * m_indexBufferWritePos, sizeof(IndexType) * indexSize, pSrc);
