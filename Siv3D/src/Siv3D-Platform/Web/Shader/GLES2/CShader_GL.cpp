...
namespace s3d
{
	...
	bool CShader_GL::init()
	{
		...
		// [エンジン PS] をロード
		{
			...
			m_enginePSs << PixelShader(Resource(U"engine/shader/GLSL1.0/copy.frag"), {});
			m_enginePSs << PixelShader(Resource(U"engine/shader/GLSL1.0/gaussian_blur_9.frag"), PSUniformBindings);
	...
	VertexShaderID CShader_GL::createVSFromSource(const String& source, const Array<ConstantBufferBinding>& bindings)
	{
		...
		vertexShader->setUniformBindings(bindings);
	...
	PixelShaderID CShader_GL::createPSFromSource(const String& source, const Array<ConstantBufferBinding>& bindings)
	{
		...
		pixelShader->setUniformBindings(bindings);
	...
	void CShader_GL::setPSSamplerUniforms(const PixelShaderID handleID)
	{
		m_pixelShaders[handleID]->setPSSamplerUniform();
	}
	
	void CShader_GL::setVSUniforms(const VertexShaderID handleID, const void* uniformBuffer)
	{
		m_vertexShaders[handleID]->loadUniformVariables(uniformBuffer);
	}

	void CShader_GL::setPSUniforms(const PixelShaderID handleID, const void* uniformBuffer)
	{
		m_pixelShaders[handleID]->loadUniformVariables(uniformBuffer);
	}
}
